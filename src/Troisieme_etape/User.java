...
public class User {
	...
	public static Standard newContact(String[] texteBiographiques) {
		...
		System.out.println("Ecco il Nome:" + nom);
		...
		try {
			...
			case NOM:
				for (Standard c : arrayContact)
					if (c.getNom().toLowerCase().startsWith(stringRecherche))
			...
			case ADRESSE:
				for (Standard c : arrayContact)
					if (c.getAdresse().toLowerCase().startsWith(stringRecherche))
						index.add(arrayContact.indexOf(c));
				break;
			...
			case PROFESSION:
				for (Standard c : arrayContact)
					if (c.getProfession().toLowerCase().startsWith(stringRecherche))
						index.add(arrayContact.indexOf(c));
				break;
			...
			case SIGNE_ZODIAQUE:
				for (Standard c : arrayContact) {
					Amis a = (Amis) c;
					if (a.getSigneZodiacal().toLowerCase().startsWith(stringRecherche))
						index.add(arrayContact.indexOf(c));
				}
				break;
			...
			case LIEN_PARENTE:
				for (Standard c : arrayContact) {
					Famille f = (Famille) c;
					if (f.getLienParent().toLowerCase().startsWith(stringRecherche))
						index.add(arrayContact.indexOf(c));
				}
				break;
			...
			case FONCTION:
				for (Standard c : arrayContact) {
					...
					if (p.getFonction().toLowerCase().startsWith(stringRecherche))
						index.add(arrayContact.indexOf(c));
				}
	...
	public Standard editContact(String nom, String[] texteBiographiques) {
		...
		// VERIFICATION RECHERCHE NOM POUR MODIFICATION
		if (index == -1) {
